"""
Kafka integration for the Agent Orchestrator.
Service starts and runs without Kafka — logs a warning on failure.
"""
...
async def init_kafka(bootstrap_servers: str) -> None:
    ...
    try:
        ...
        logger.warning(
            "Kafka unavailable (%s) — running without Kafka. Messages will be dropped.",
            exc,
        )
        _kafka_available = False
...
async def publish(topic: str, message: dict[str, Any], key: Optional[str] = None) -> bool:
    """Publish a message to Kafka. Returns True on success, False on failure."""
